...
국립공원, 한라산, 서울 교통, 고속도로, 하천 수위 감시처럼 **공식적으로 공개된 실시간 영상**을 한 화면에 붙인다. 골목 방범은 실시간 여부와 관계없이 접근 가능한 위치 지도, 설치 데이터, 공개 영상 자료와 열람 안내도 모은다. 현재 연결된 골목 방범 실시간 영상은 없다.
...
**[방범·공개자료](http://localhost:3000/resources)** 페이지에 확인한 공식 접근 경로 6개를 모았다. 메인 화면의 `방범 위치·영상 자료·열람 안내` 버튼으로도 열린다. 자료 종류와 지역·기관·내용으로 검색할 수 있다.
...
| 생활안전지도 | 공식 지도 |
| AI Hub 공원 CCTV 영상 자료 | 연출·비식별화 학습 영상 소개, 샘플·파일 목록. 원본 이용 조건은 제공처 확인 |
